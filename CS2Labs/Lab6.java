...
class Lab6 {
    ...
    static int binarySearchDuration(Airplane[] airplanes, int BSDuration) {
        ...
        while(low <= high){
            ...
            else if(BSDuration > airplanes[mid].getDuration()){
                low = mid + 1;
            }
            else{
                System.out.println("Airplane with Duration " + BSDuration + " found at position " + mid);
    ...
    static int binarySearchByCapacity(Airplane[] airplanes, int BSCapacity) {
        ...
        while(low <= high){
            ...
            else if(BSCapacity > airplanes[mid].getCapacity()){
                low = mid + 1;
            }
            else{
    ...
    static int linearSearchByDuration(Airplane[] airplanes, int LSDuration) {
        for(int i = 0; i < airplanes.length; i++){
            if(airplanes[i].getDuration() == LSDuration){
                System.out.println("Airplane with capacity " + LSDuration + " found at position " + i);
                System.out.println(airplanes[i].toString());
            }
        }
        return -1;
